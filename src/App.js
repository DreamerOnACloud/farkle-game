...
const initializeDice = () => Array(6).fill().map(() => ({ value: Math.floor(Math.random() * 6) + 1, active: true }));
...
const App = () => {
  ...
  const reroll = () => {
    ...
    const newDice = dice.map((die, idx) => {
      if (!scoringDiceIndices.includes(idx + 1)) {
  ...
  return (
    <div className="App">
      ...
      <Dice diceValues={dice.map(die => die.value)} />
      <Controls
        reroll={reroll}
        score={score}
        endTurn={endTurn}
